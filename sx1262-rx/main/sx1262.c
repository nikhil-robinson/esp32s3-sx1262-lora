...
static int SX126x_ANTENA_SWITCH;
...
void LoRaInit(gpio_num_t miso, gpio_num_t mosi, gpio_num_t sclk, gpio_num_t nss, gpio_num_t rst, gpio_num_t busy, gpio_num_t antena_sw)
{
	ESP_LOGI(TAG, "CONFIG_MISO_GPIO=%d", miso);
	ESP_LOGI(TAG, "CONFIG_MOSI_GPIO=%d", mosi);
	ESP_LOGI(TAG, "CONFIG_SCLK_GPIO=%d", sclk);
	ESP_LOGI(TAG, "CONFIG_NSS_GPIO=%d", nss);
	ESP_LOGI(TAG, "CONFIG_RST_GPIO=%d", rst);
	ESP_LOGI(TAG, "CONFIG_BUSY_GPIO=%d", busy);
	ESP_LOGI(TAG, "CONFIG_ANTENA_SW_GPIO=%d", antena_sw);

	SX126x_SPI_SELECT = nss;
	SX126x_RESET = rst;
	SX126x_BUSY	= busy;
	SX126x_ANTENA_SWITCH= antena_sw;
	...
	if (SX126x_ANTENA_SWITCH != -1) {
		gpio_reset_pin(SX126x_ANTENA_SWITCH);
		gpio_set_direction(SX126x_ANTENA_SWITCH, GPIO_MODE_OUTPUT);
	}
...
void SetRxEnable(void)
{
	...
	if ((SX126x_ANTENA_SWITCH != -1)) {
		gpio_set_level(SX126x_ANTENA_SWITCH, HIGH);
	}
}
...
{
	if (debugPrint) {
		ESP_LOGI(TAG, "SetTxEnable");
	}
	if (SX126x_ANTENA_SWITCH != -1){
		gpio_set_level(SX126x_ANTENA_SWITCH, LOW);
	}
}
